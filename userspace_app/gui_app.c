 ...
 */
 ...
 static void on_usb_mgr_mount(GtkButton *btn, gpointer data) {
 ...
 
     {
        ...
        /* Kiểm tra device đã được mounted chưa (udisks2 có thể đã auto-mount) */
        {
            char proc_line[1024];
            char already_mp[256] = {0};
            FILE *mfp = fopen("/proc/mounts", "r");
            if (mfp) {
                while (fgets(proc_line, sizeof(proc_line), mfp)) {
                    char mdev[256], mpath[256];
                    if (sscanf(proc_line, "%255s %255s", mdev, mpath) == 2 &&
                        strcmp(mdev, device) == 0) {
                        strncpy(already_mp, mpath, sizeof(already_mp) - 1);
                        break;
                    }
                }
                fclose(mfp);
            ...
            if (already_mp[0]) {
                usb_mgr_log(st, "%s already mounted at %s (reusing)", device, already_mp);
                strncpy(st->current_mount, already_mp, sizeof(st->current_mount) - 1);
                st->current_mount[sizeof(st->current_mount) - 1] = '\0';
                log_audit(st->app->logged_in_user, "Reused mount %s at %s", device, already_mp);
        ...
        /* Detect filesystem type thực tế bằng blkid để driver dùng đúng type */
        char fs_type[32] = "vfat";
        {
            char blkid_cmd[256];
            snprintf(blkid_cmd, sizeof(blkid_cmd),
                     "blkid -o value -s TYPE %.200s 2>/dev/null", device);
            FILE *bp = popen(blkid_cmd, "r");
            if (bp) {
                char detected[32] = {0};
                if (fgets(detected, sizeof(detected), bp)) {
                    detected[strcspn(detected, "\n\r")] = '\0';
                    if (detected[0])
                        strncpy(fs_type, detected, sizeof(fs_type) - 1);
                }
                pclose(bp);
            }
        }

        usb_mgr_log(st, "Mounting %s (fs=%s)...", device, fs_type);
        char actual_mp[256] = {0};
        ret = usb_mount_detect(device, mount_point, fs_type, "rw",
...
static void on_usb_mgr_unmount(GtkButton *btn, gpointer data) {
    ...
    GtkWidget *confirm = gtk_message_dialog_new(GTK_WINDOW(st->dialog),
        GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    if (response != GTK_RESPONSE_YES) {
        usb_mgr_log(st, "Unmount cancelled");
        return;
    }

    usb_mgr_log(st, "Unmounting %s ...", st->current_mount);

    /* Luôn thử qua driver trước để kernel có thể log và xử lý */
    if (usb_driver_available()) {
        usb_mgr_log(st, "  -> sending ioctl to usb_bridge driver");
        ret = usb_unmount(st->current_mount);
        if (ret == 0) {
            usb_mgr_log(st, "  -> driver unmount OK");
            goto unmount_done;
        }
        usb_mgr_log(st, "  -> driver unmount failed (ret=%d), fallback to system", ret);
    } else {
        usb_mgr_log(st, "  -> driver not available, using system unmount");
    }

    /* Fallback: udisksctl / umount2 */
    ret = usb_unmount_system(st->current_mount);

unmount_done:
    if (ret == 0) {
        log_audit(st->app->logged_in_user, "Unmounted USB at %s", st->current_mount);
        usb_mgr_log(st, "Unmount OK  [%s]", st->current_mount);
        st->current_mount[0] = '\0';
        gtk_list_store_clear(st->file_store);
        usb_mgr_update_status(st);
    } else {
        usb_mgr_log(st, "Unmount FAILED (ret=%d)", ret);
    }
}
 ...
 static void on_usb_mgr_copy_to(GtkButton *btn, gpointer data) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        usb_mgr_log(st, "Copying %s -> USB:%s/%s ...", src, st->current_mount, base);
        unsigned int copied = 0;
        int ret = usb_copy_to_device(src, dst, &copied);
        if (ret == 0) {
            st->total_writes++;
            usb_mgr_update_status(st);
            usb_mgr_refresh_files(st);
            usb_mgr_log(st, "Copy OK: %u bytes written to USB", copied);
            log_audit(st->app->logged_in_user, "USB Copy To: %s", base);
        } else {
            /* Fallback: libc copy khi driver từ chối */
            usb_mgr_log(st, "Driver copy failed (ret=%d), trying libc fallback...", ret);
            ...
            if (fsrc && fdst) {
                ...
                usb_mgr_refresh_files(st);
                usb_mgr_log(st, "Copy OK (libc): %u bytes written to USB", copied);
                log_audit(st->app->logged_in_user, "USB Copy To (libc): %s", base);
            } else {
                if (fsrc) fclose(fsrc);
                if (fdst) fclose(fdst);
                usb_mgr_log(st, "Copy FAILED: cannot open src or dst file");
            }
        }
        g_free(base);
        g_free(src);
 ...
 static void on_usb_mgr_copy_from(GtkButton *btn, gpointer data) {
    ...
    gtk_file_chooser_set_do_overwrite_confirmation(GTK_FILE_CHOOSER(dialog), TRUE);
    /* Chỉ cho phép lưu .txt / .csv để driver không bị reject */
    GtkFileFilter *save_filter = gtk_file_filter_new();
    gtk_file_filter_set_name(save_filter, "Text/CSV (*.txt, *.csv)");
    gtk_file_filter_add_pattern(save_filter, "*.txt");
    gtk_file_filter_add_pattern(save_filter, "*.csv");
    gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(dialog), save_filter);

    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        char *dst = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
        usb_mgr_log(st, "Copying USB:%s -> %s ...", filename, dst);
        unsigned int copied = 0;
        int ret = usb_copy_from_device(src, dst, &copied);
        if (ret == 0) {
            st->total_reads++;
            usb_mgr_update_status(st);
            usb_mgr_log(st, "Copy OK: %u bytes saved to %s", copied, dst);
            ...
            if (fsrc && fdst) {
                ...
                fclose(fsrc); fclose(fdst);
                usb_mgr_log(st, "Copy OK (libc): %u bytes saved to %s", copied, dst);
                log_audit(st->app->logged_in_user, "USB Copy From (libc): %s", filename);
            } else {
                if (fsrc) fclose(fsrc);
                if (fdst) fclose(fdst);
                usb_mgr_log(st, "Copy FAILED: cannot open USB:%s or dst %s", filename, dst);
            }
        }
        g_free(dst);
    }
 ...
 static void on_open_usb_manager(GtkButton *btn, gpointer user_data) {
     ...
     /*
      * This dialog is the highest-level USB workflow in the app:
      * GUI widgets -> usb_file.c policy/fallback layer -> usb_driver_client.c
      * -> /dev/usb_bridge when the driver is available.
      */
 ...
 static GtkWidget *build_dashboard_view(AppState *app) {
     /*
      * Dashboard construction is long because the whole main screen is declared
      * in one place: sidebar, table, form, export actions, and status bar.
      * AppState stores the important widgets so callbacks can update them later.
      */
