...
echo -e "\n${YELLOW}[STEP 1] Build Kernel Module${NC}"
run_test "Build string_norm.ko" "make -C $KERNEL_DIR"

echo -e "\n${YELLOW}[STEP 2] Load Kernel Module${NC}"
...
echo -e "\n${YELLOW}[STEP 3] Normalize Mock Tests (no driver needed)${NC}"
...
echo -e "\n${YELLOW}[STEP 4] SHA-256 Authentication Tests${NC}"
...
echo -e "\n${YELLOW}[STEP 5] Student CRUD Unit Tests${NC}"
...
echo -e "\n${YELLOW}[STEP 6] Driver Integration Tests${NC}"
...
echo -e "\n${YELLOW}[STEP 7] Kernel Log Check${NC}"
echo "--- Kernel messages ---"
dmesg | grep -E "(string_norm|USB_DRIVER)" | tail -20 || true

echo -e "\n${YELLOW}[STEP 8] Cleanup${NC}"
run_test "Unload driver" \
	"sudo rmmod string_norm 2>/dev/null; \
